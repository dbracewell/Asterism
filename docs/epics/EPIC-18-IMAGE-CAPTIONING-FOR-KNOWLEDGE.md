...
- Captioning is disabled by default. An admin configures exactly one active
  ...
  - **Local mode:** a pinned, explicitly provisioned SmolVLM2 artifact bundle.
...
- Local mode is opt-in and runs bounded background work on CPU. It has separate
  model cache, integrity verification, readiness diagnostics, concurrency, and
  memory guidance. It never downloads weights during a caption request.
...
- [ ] US-18.1-T1: Define caption-provider protocols, typed caption/revision
      records, status/error taxonomy, configuration, and provider/local
      selection validation.
- [ ] US-18.1-T2: Add a pinned local SmolVLM2 adapter with explicit local cache,
      integrity checks, CPU-only execution, no remote code, bounded concurrency,
      and clear readiness failures.
...
- [ ] US-18.1-T4: Add unit/smoke tests for provider selection, local artifact
...
- [ ] US-18.2-T2: Add an admin configuration API/UI that selects disabled,
      provider model, or local SmolVLM2 mode and displays readiness/hardware
      guidance without exposing credentials.
