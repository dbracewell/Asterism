...
export function KnowledgeBaseManager() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const { data } = await api.knowledgeBaseGetMany({
  ...
  const create = async (event: FormEvent) => {
    ...
    try {
      ...
      setError(
        "Unable to create this knowledge base. Its name may already be in use.",
      );
    }
  };

  const remove = async (base: KnowledgeBase) => {
    if (!window.confirm(`Delete “${base.name}” and its indexed documents?`))
      return;
    try {
  ...
  return (
    <main className="mx-auto mt-12 w-full max-w-4xl space-y-6 p-6">
      <header>
        <h1 className="text-2xl font-semibold">Knowledge bases</h1>
        <p className="text-muted-foreground">
          Private documents available to assigned agents.
        </p>
      </header>
      <form
      ...
      {loading ? (
        ...
        <p className="text-muted-foreground">
          No knowledge bases yet. Create one to begin indexing private
          documents.
        </p>
      ) : (
