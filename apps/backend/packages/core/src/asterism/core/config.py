...
class Config:
    ...
    def LLM_NATIVE_TOOL_SUPPORT(self) -> bool:
        val = self._get_value("LLM_NATIVE_TOOL_SUPPORT", "true")
        return str(val).lower() == "true" if isinstance(val, str) else bool(val)

    @property
    def LLM_SUPPORTS_SYSTEM_PROMPT(self) -> bool:
        val = self._get_value("LLM_SUPPORTS_SYSTEM_PROMPT", "true")
        return str(val).lower() == "true" if isinstance(val, str) else bool(val)

    @property
    def LLM_SUPPORTS_STRUCTURED_OUTPUT(self) -> bool:
        val = self._get_value("LLM_SUPPORTS_STRUCTURED_OUTPUT", "true")
        return str(val).lower() == "true" if isinstance(val, str) else bool(val)

    @property
    def LLM_CONTEXT_WINDOW(self) -> int:
        return int(self._get_value("LLM_CONTEXT_WINDOW", 32000))
