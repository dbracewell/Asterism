...
from asterism.common import AuthedUser, LLMModel
...
from asterism.llm.client import LLMClient, LLMEvent, LLMEventType
from asterism.llm.draft import get_draft_model
from asterism.models.message import MessageStatus
from asterism.registries.tool import (
    ToolCall,
    ToolResult,
    tool_registry,
)
...
class WebSocketChatConnection:
    ...
    async def _process_messages(self, processing_tools: bool = False) -> None:
        try:
            ...
            # Update the last user message to be completed in
            # case it is not (tool call)
            ...
            await self._process_tool_calls(
                ai_message_model, response.tool_calls, model
            )
    ...
    async def _process_tool_calls(
        ...
        tasks = [
            tool_registry.invoke_tool(tool_call=tc, user=self.user)
        ...
        try:
            ...
            while True:
                ...
                if "message" not in message_data or "model" not in message_data:
                    raise ValueError(
                        f"websocket request missing data {message_data}"
                    )
